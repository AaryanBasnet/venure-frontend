...
export default function ChatList({ chats, activeChat, onSelect }) {
  ...
  const getUnreadCount = (chat) => {
    ...
    // For now, returning a random number for demo purposes
    return chat.unreadCount || 0;
  };

  const getParticipantNames = (participants) => {
    return participants
      .filter(p => p.role !== 'owner') // Exclude owner from display
  ...
  const isOnline = (chat) => {
    ...
    return chat.isOnline || Math.random() > 0.5; // Random for demo
  };
  ...
  return (
    <div className="h-full bg-white">
      ...
      <div className="overflow-y-auto h-full">
        {chats.map((chat) => {
          const participantNames = getParticipantNames(chat.participants);
      ...
      <div className="p-4 border-t border-gray-100 bg-gray-50">
        <div className="flex items-center justify-between text-sm text-gray-600">
          ...
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-400 rounded-full"></div>
            <span>{chats.filter(chat => isOnline(chat)).length} online</span>
          </div>
        </div>
      </div>
    </div>
