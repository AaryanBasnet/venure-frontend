...
const BookingSummary = ({
  venueCost, selectedDate, selectedTimeSlot, timeSlots,
  currentStep, numberOfGuests, eventType,
  selectedAddons, addOns, calculateTotal
}) => {
  return (
      ...
      <div>
        ...
        <div className="flex items-center mb-6">
          <div className="w-12 h-12 bg-gray-200 rounded-full flex items-center justify-center mr-4">
            <i className="fas fa-building text-gray-600"></i> {/* Placeholder icon */}
          </div>
          <div>
            <p className="font-semibold text-gray-800">Royal Palace Banquet</p>
            <p className="text-sm text-gray-600"><i className="fas fa-map-marker-alt mr-1"></i>Downtown, Mumbai</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between text-gray-700">
            ...
            <span className="font-medium">
              {selectedDate ? selectedDate.toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' }) : 'N/A'}
            </span>
          </div>
          <div className="flex justify-between text-gray-700">
            <span>Time Slot:</span>
            <span className="font-medium">
              {selectedTimeSlot ? timeSlots.find(slot => slot.id === selectedTimeSlot)?.label : 'N/A'}
            </span>
          </div>
          <div className="flex justify-between text-gray-700">
              ...
              <div className="pt-2">
                ...
                <ul className="text-sm text-gray-600 pl-4 list-disc space-y-1">
                  {selectedAddons.map(addonId => {
                    ...
                    return addon ? (
                      <li key={addon.id}>
                        {addon.name}: ₹{addon.price.toLocaleString()}{addon.perPerson && ` x ${numberOfGuests || 0} guest(s)`}
                      </li>
                    ) : null;
                  })}
                </ul>
