...
import { FiPlus, FiMoreVertical, FiTrash2, FiAlertTriangle } from 'react-icons/fi';
import { MdBlock } from 'react-icons/md';
import SearchInput from '../../components/common/SearchInput';
import AddOwnerModal from '../../components/specific/AddOwnerModal';
...
const AdminVenueOwnerPage = () => {
  ...
  const handleUpdateStatus = async (ownerId, currentStatus) => {
    ...
    setOwners(prev => prev.map(o => o._id === ownerId ? {...o, status: newStatus } : o));
    setOpenActionMenu(null);
  };
                        ...
                        <tr key={owner._id} className="hover:bg-gray-50 transition-colors">
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center gap-3">
                                    <img className="h-10 w-10 rounded-full object-cover" src={owner.avatar} alt={owner.name} />
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600 text-center">{owner.venuesManaged}</td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                                <div className="relative inline-block text-left">
                                    <button onClick={() => setOpenActionMenu(openActionMenu === owner._id ? null : owner._id)}>
                                        <FiMoreVertical size={20} className="text-gray-500 hover:text-gray-800" />
                                    </button>
                                    {openActionMenu === owner._id && (
                                        <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
                                            <div className="py-1">
                                                <button onClick={() => handleUpdateStatus(owner._id, owner.status)} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                                                    <MdBlock/> {owner.status === 'Active' ? 'Block' : 'Unblock'}
                                                </button>
                                                <button onClick={() => handleDeleteClick(owner)} className="w-full text-left flex items-center gap-3 px-4 py-2 text-sm text-red-600 hover:bg-red-50">
                                                    <FiTrash2/> Delete
                                                </button>
                                            </div>
                                        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex justify-center items-center z-50">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <div className="mt-6 flex justify-center space-x-4">
              <button className="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50" onClick={() => setOwnerToDelete(null)}>
                Cancel
              </button>
              <button className="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-md hover:bg-red-700" onClick={confirmDelete}>
                Delete
              </button>
            </div>
          </div>
